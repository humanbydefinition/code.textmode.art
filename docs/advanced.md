---
title: Advanced Features
description: Compatibility index for the former Advanced features page, now split into focused textmode.js capability guides.
---

# Advanced features

This page has been split into focused capability guides. ( •̀ᴗ•́ )و ̑̑

## Rendering techniques

- [Layers and compositing](/docs/layers-and-compositing)
- [Filters](/docs/filters)
- [Framebuffers](/docs/framebuffers)
- [Shaders](/docs/shaders)

## 3D

- [3D drawing](/docs/3d-drawing)
- [Cameras and projection](/docs/cameras-and-projection)
- [Lighting](/docs/lighting)

## Runtime extension

- [Loading and errors](/docs/loading-and-errors)
- [Plugins](/docs/plugins)

## Media

- [Media conversion](/docs/media-conversion)

The old URL is kept for compatibility. New docs should link to the focused pages above.
